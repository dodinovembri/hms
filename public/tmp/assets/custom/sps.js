        ...
        $.each(data, function(i, obj){
          // console.log(obj['id'])
           if (Number(id) === obj['id']) {
                ...
                $('.body-detail').html('')
                $('.body-detail').append('<div class="container"><br><strong>Customer Name </strong>'+' &nbsp;&nbsp;&nbsp;<span class="owner_name_c underline">'+obj.owner_outlet_firstname+'</span><text class="pull-right"><strong>Otlet Type</strong>'+'  &nbsp;&nbsp;&nbsp;<span>'+outlet_type+'</span></text></div>\
                  <br>')
...
function chooseproduct(counter, data) {
  ...
  return    '<td><div class="form-group"><select style="width:243px; font-size:11px;" id="name-'+counter+'"  name="product_id[]" class="full-width form-control select-product" onChange="appendproduct('+counter+', this.value, this)" data-live-search="true" data-live-search-style="startsWith" class="selectpicker" ></select></div></td>' 
          ...
          + '<td><input name="qty[]" style="width:50px; font-size:11px;" id="qty-'+counter+'"  type="text" value ="" class="form-control" onkeyup="hitungTotalBayar(this.value, $(\'#unit_price-'+counter+'\').val(), $(\'#discount-'+counter+'\').val(), \'#subtotal-'+counter+'\', '+counter+',this)" /></td>'
          + '<td><input name="unit_price[]" style="width:70px; font-size:11px; color:black;" id="unit_price-'+counter+'" readonly="readonly" type="text" value ="" class="form-control" /></td>'
          + '<td><input name="price[]" style="width:90px; font-size:11px; color:black;" id="price-'+counter+'" readonly="readonly" type="text" value ="" class="form-control" /></td>'
          + '<td><input name="discount[]" style="width:70px; font-size:11px;" id="discount-'+counter+'"  type="text" value ="0" class="form-control discount" onkeyup="hitungSubtotal(this.value, $(\'#price-'+counter+'\').val(), $(\'#discount-'+counter+'\').val(), \'#subtotal-'+counter+'\', '+counter+',this)" /></td>'
          + '<td><input name="total_discount[]" style="width:70px; font-size:11px; color:black;" id="total_discount-'+counter+'" readonly="readonly" type="text" value ="" class="form-control" /></td>'
          + '<td><input name="subtotal[]" id="subtotal-'+counter+'" value="0" readonly="readonly" type="text" value ="" class="form-control totalprice" style="width:90px; font-size:11px; color:black;" /></td>'
          + '<td width="50"><a href="#" onclick="delrow(this)" id="del_s-'+counter+'" title="Delete"><i class="glyphicon glyphicon-remove-sign"></i></a></td>'
...
function hitungTotalBayar(qty, price, discount, total, counter, selector)
{
    ...
    var i=1;
        $(function () {
            $('.tableDataP .body-product tr').each(function (a, b) {
                ...
                product[i]  = { name: name, sku: sku, qty:qty, unit_price:unit_price, price:price, discount:discount, total_discount:total_discount,total_price:total_price, mwuom:mwuom, product_id:product_id,productfe_id:productfe_id };
            i++;
                   ...
                   $(tr).find("[name='productfe_id[]']").val(obj['productfe_id'])
                    ...
                    if(obj['pricebl'] != ''){
                      ...
                      $.each(obj.pricebl, function(a, b){ 
                         $(tr).find("[name='unit_price[]']").val(b['price_package_w'])
